...
export default function Viewport({mode,room,reset,daylight,onReady,onPosition}:{mode:ViewMode;room:string;reset:number;daylight:boolean;onReady:(n:number)=>void;onPosition:(x:number,z:number)=>void}){
 ...
 useEffect(()=>{
  ...
  const scene=new THREE.Scene();scene.background=new THREE.Color('#bfcbd0');const camera=new THREE.PerspectiveCamera(58,1,.035,160);const model=buildApartment();scene.add(model.group);let disposed=false;let releaseTextures=()=>{};const lighting=createApartmentLighting(renderer,scene,model,{probeSize:128});applyReferenceMaterials(model.group,()=>disposed).then(release=>{releaseTextures=release;lighting.refresh()}).catch(()=>console.warn("Reference materials could not load; using procedural fallback."));
