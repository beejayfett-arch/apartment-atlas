...
import { createFurnitureEditor } from './furniture-editor';
import { applyExterior } from './exterior';
export type ViewMode='walk'|'dollhouse'|'photos'|'edit';
export default function Viewport({mode,room,reset,daylight,onReady,onPosition}:{mode:ViewMode;room:string;reset:number;daylight:boolean;onReady:(n:number)=>void;onPosition:(x:number,z:number)=>void}){
 ...
 useEffect(()=>{
  ...
  const pipeline=createRenderPipeline(renderer,scene,camera,mobile);
  ...
  function setView(m:ViewMode,r:string){currentMode=m;renderDirty=true;renderer.shadowMap.needsUpdate=true;controls.enabled=m==='dollhouse'||m==='edit';editor.setActive(m==='edit');model.ceiling.visible=m==='walk';model.context.visible=m==='walk';walkTarget=null;keys.clear();if(m==='dollhouse'||m==='edit'){if(m==='edit')camera.position.set(3.55,17,9);else camera.position.set(10.8,13.4,14.6);controls.target.set(2.8,0,5.2);camera.fov=45;camera.lookAt(controls.target);controls.update()}else{const p=model.rooms.find(v=>v.id===r)||model.rooms[0];camera.position.set(...p.position);const delta=new THREE.Vector3(...p.target).sub(camera.position);yaw=Math.atan2(-delta.x,-delta.z);pitch=Math.atan2(delta.y,Math.hypot(delta.x,delta.z));camera.fov=68;rotation()}camera.updateProjectionMatrix();report.current(camera.position.x,camera.position.z)}
  function safe(x:number,z:number){const inBody=x>.19&&x<6.92&&z>.19&&z<7.73;const inKitchen=x>4.98&&x<6.92&&z>=7.73&&z<10.8;const balcony=x>-1.2&&x<.25&&z>5.1&&z<7.7;if(!inBody&&!inKitchen&&!balcony)return false;return !editor.collides(x,z)&&!model.colliders.some(c=>x>c.minX-.15&&x<c.maxX+.15&&z>c.minZ-.15&&z<c.maxZ+.15)}
  function move(dx:number,dz:number){if(!dx&&!dz)return;const x=camera.position.x+dx,z=camera.position.z+dz;if(safe(x,camera.position.z))camera.position.x=x;if(safe(camera.position.x,z))camera.position.z=z}
  ...
  function resize(){const w=el.clientWidth,h=el.clientHeight;renderer.setSize(w,h);pipeline.resize(w,h);camera.aspect=w/h;camera.updateProjectionMatrix();renderDirty=true};const observer=new ResizeObserver(resize);observer.observe(el);resize();
  function animate(t:number){frame=requestAnimationFrame(animate);const dt=Math.min((t-last)/1000,.04);last=t;if(document.hidden)return;if(currentMode==='dollhouse'||currentMode==='edit')controls.update();else if(currentMode==='walk'){let f=(keys.has('w')||keys.has('arrowup')?1:0)-(keys.has('s')||keys.has('arrowdown')?1:0);let r=(keys.has('d')||keys.has('arrowright')?1:0)-(keys.has('a')||keys.has('arrowleft')?1:0);const speed=1.55*dt/(f&&r?Math.SQRT2:1);move((-Math.sin(yaw)*f+Math.cos(yaw)*r)*speed,(-Math.cos(yaw)*f-Math.sin(yaw)*r)*speed);if(walkTarget){const dir=walkTarget.clone().sub(camera.position);if(dir.length()<.08)walkTarget=null;else{dir.normalize().multiplyScalar(dt*1.8);move(dir.x,dir.z)}}}if(currentMode!=='photos'&&(renderDirty||lighting.pending>0||!camera.position.equals(renderedPosition)||!camera.quaternion.equals(renderedQuaternion))){lighting.step();pipeline.render();renderedPosition.copy(camera.position);renderedQuaternion.copy(camera.quaternion);renderDirty=false;}if(++count%20===0)report.current(camera.position.x,camera.position.z)}
  let reflectionTimer:ReturnType<typeof setTimeout>|undefined;let lastFurniture='';
  const refreshFurniture=(event:Event)=>{const items=(event as CustomEvent).detail.items;const signature=JSON.stringify(items);if(signature===lastFurniture)return;lastFurniture=signature;clearTimeout(reflectionTimer);reflectionTimer=setTimeout(()=>{if(!disposed){lighting.syncFurniture();renderDirty=true}},350)};window.addEventListener('atlas-editor-state',refreshFurniture);
  const editor=createFurnitureEditor(model,scene,camera,canvas,()=>{renderDirty=true;renderer.shadowMap.needsUpdate=true},enabled=>{controls.enabled=enabled&&(currentMode==='edit'||currentMode==='dollhouse')});
