...
export function buildApartment() {
  ...
  function canvasTexture(kind:string, base:string) {
    ...
    if(kind==='tile') {
      for(let i=0;i<6500;i++){const x=rand()*512,y=rand()*512;ctx.fillStyle=`rgba(113,112,106,${rand()*.026})`;ctx.beginPath();ctx.ellipse(x,y,rand()*42+3,rand()*11+1,rand()*3,0,7);ctx.fill()}
      for(let i=0;i<35;i++){ctx.strokeStyle=`rgba(125,127,125,${rand()*.07})`;ctx.lineWidth=rand()*3;ctx.beginPath();const x=rand()*512,y=rand()*512;ctx.moveTo(x,y);ctx.bezierCurveTo(x+60,y-10,x+120,y+40,x+200,y-65);ctx.stroke()}
      ctx.strokeStyle='#929590';ctx.lineWidth=2.6;ctx.strokeRect(0,0,512,512);
    } else if(kind==='wood') {
  ...
  function mat(name:string,color:string,roughness=.72,metalness=0,kind?:string){
    const m=new THREE.MeshStandardMaterial({color,roughness,metalness});if(kind){m.color.set('#ffffff');m.map=canvasTexture(kind,color);m.bumpMap=m.map;m.bumpScale=kind==='fabric'||kind==='carpet'? .008:.0017}materials[name]=m;return m;
  }
  const wall=mat('paint','#eeeae1',.9),white=mat('joinery','#f6f4ed',.45),trim=mat('trim','#fcfcf7',.55),dark=mat('charcoal','#25282a',.45),black=mat('black hardware','#202122',.35,.4),steel=mat('stainless','#a4adb0',.27,.82),glass=new THREE.MeshPhysicalMaterial({color:'#d0e3e4',roughness:.06,metalness:0,transparent:true,opacity:.17,side:THREE.DoubleSide,depthWrite:false}),wood=mat('oak','#a97a4d',.52,0,'wood'),walnut=mat('walnut','#805838',.5,0,'wood'),tile=mat('marble tile','#dedfd8',.62,0,'tile'),carpet=mat('carpet','#b9afa0',.97,0,'carpet'),leather=mat('leather','#343431',.6,0,'leather'),linen=mat('linen','#c9bea9',.96,0,'fabric'),cushion=mat('cushion','#967765',.97,0,'fabric'),throwMat=mat('ochre throws','#a86c3c',1,0,'fabric'),rug=mat('rug','#c9c7bb',1,0,'rug'),floral=mat('floral duvet','#e4dce0',1,0,'floral'),brick=mat('balcony brick','#b88d67',.98,0,'brick');
  ...
  function drapedThrow(x:number,z:number,w:number,d:number,turn=false){
    const g=new THREE.PlaneGeometry(w,d,48,40),pos=g.attributes.position;
    for(let i=0;i<pos.count;i++){const u=pos.getX(i),v=pos.getY(i);const front=Math.max(0,(-u/w-.4)/.1);const ripple=.007*Math.sin(v*57+u*7)+.006*Math.sin(v*27-u*8)+.005*Math.sin(u*40+v*15);const y=.716+ripple-.13*front*front+.035*Math.exp(-Math.pow((u-w*.31)*12,2));pos.setXYZ(i,u,y,v)}g.computeVertexNormals();
    const mm=throwMat.clone();mm.side=THREE.DoubleSide;const mesh=new THREE.Mesh(g,mm);mesh.position.set(x,0,z);if(turn)mesh.rotation.y=-Math.PI/2;mesh.castShadow=true;mesh.receiveShadow=true;sofa.add(mesh);
  ...
  function floor(x:number,z:number,w:number,d:number,m:THREE.MeshStandardMaterial,repeat=.43,rotate=false){const mm=m.clone();if(m.map){mm.map=m.map.clone();mm.map.repeat.set(w/repeat,d/repeat);if(rotate){mm.map.center.set(.5,.5);mm.map.rotation=Math.PI/4;}mm.bumpMap=mm.map}const mesh=box(x,-.065,z,w,.13,d,mm);return mesh}
  ...
  wallSeg(2.7,1.75,.14,3.5);wallSeg(4.4,1.75,.14,3.5);wallSeg(3.55,2.5,1.7,.13,.45,2.325);wallSeg(2.86,2.5,.32,.13);wallSeg(4.25,2.5,.3,.13);
  ...
  function doorway(x:number,z:number,w:number,rotation=0,door=true){const g=new THREE.Group();g.position.set(x,0,z);g.rotation.y=rotation;group.add(g);for(const xx of[-w/2,w/2]){box(xx,1.08,0,.06,2.16,.17,trim,g);box(xx,1.09,.1,.035,2.18,.025,trim,g)}box(0,2.16,0,w+.07,.065,.17,trim,g);
    if(door){const hinge=new THREE.Group();hinge.position.set(-w/2,0,0);hinge.rotation.y=-Math.PI*.44;g.add(hinge);box(w/2,1.05,0,w-.06,2.1,.038,white,hinge);box(w*.82,1,.038,.12,.025,.024,steel,hinge);for(const yy of[.25,1,1.8])box(.025,yy,0,.025,.07,.06,steel,hinge);}
  }
  doorway(2.19,3.5,.7);doorway(5.01,3.5,.86);doorway(3.56,2.5,1.08);doorway(5.96,9.7,.88);doorway(4.15,7.9,.9,Math.PI,false);box(4.15,1.04,7.94,.82,2.08,.045,white,outerWalls);box(4.46,1.04,7.9,.11,.028,.05,steel,outerWalls);
  ...
  for(let i=0;i<3;i++){softCushion(3.58,.57,5.23+i*.79,.77,.22,.75,linen,sofa);const c=softCushion(3.83,.91,5.23+i*.79,.25,.67,.72,cushion,sofa);c.rotation.z=.21+i*.025;c.rotation.x=(i-1)*.045;box(3.157,.33,5.23+i*.79,.005,.39,.004,linen,sofa)}
  for(let i=0;i<2;i++){softCushion(1.99+i*.8,.57,7.16,.75,.22,.72,linen,sofa);const c=softCushion(1.99+i*.8,.91,7.44,.73,.67,.25,cushion,sofa);c.rotation.x=-.15}
  for(const z of[5.26,6.07,6.88])drapedThrow(3.52,z,.85,.57);
  for(const x of[1.99,2.8])drapedThrow(x,7.1,.85,.61,true);
  ...
  obstacle(3.67,6.07,.97,2.65);obstacle(2.75,7.28,2.72,.94);
  ...
  box(.12,.67,1.27,.09,1.01,1.64,wood,bedroomFurniture);box(.12,1.19,1.27,.16,.06,1.7,wood,bedroomFurniture);for(let i=0;i<10;i++)box(.12,.88,.54+i*.162,.07,.5,.055,wood,bedroomFurniture);
  ...
  box(3.95,1.25,.91,.8,1.85,.012,glass);box(4.33,1.26,.91,.018,1.88,.018,steel);for(const y of[.73,1.73])box(4.32,y,.915,.045,.065,.028,black);
  ...
  const rooms:Room[]=[
    {id:'living',name:'Living room',position:[.93,1.58,7.18],target:[2.6,1.16,5.02]},
    ...
    {id:'bedroom',name:'Bedroom 1',position:[4.88,1.58,2.69],target:[6.3,1,1.3]},
    {id:'bedroom2',name:'Bedroom 2',position:[2.12,1.58,2.56],target:[.9,1.1,.7]},
    {id:'bathroom',name:'Bathroom',position:[3.09,1.58,2.05],target:[3.8,1.25,.7]},
    {id:'balcony',name:'Balcony',position:[-.65,1.58,7.12],target:[-2.4,1.5,5.7]},
    {id:'laundry',name:'Laundry',position:[5.53,1.58,10.19],target:[6.65,.9,10.5]},
  ];
